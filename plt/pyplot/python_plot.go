...
// pythonPlot plots x-y series
func pythonPlot(x, y []float64, style ...CurveStyle) (sx, sy string) {
	uid := pythonGenUID()
	sx = io.Sf("x%d", uid)
	sy = io.Sf("y%d", uid)
...
// pythonPlotOne plots one point @ (x,y)
func pythonPlotOne(x, y float64, params ...ParamsPlot) {
